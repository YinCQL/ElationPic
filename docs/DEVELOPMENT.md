...
| 首页公开开关在 `index.php` 入口处判定 | 只拦列表展示，不碰 `/uploads/` |
...
| 跨页表单必须用 `_form_keys` 声明本页字段 | 复选框未勾选时不会提交；配置分散在多个页面后，「缺失」既可能是用户取消勾选，也可能是本页没有该字段。不声明就会互相清空（曾造成真实的数据丢失） |
| 样式按**层**拆（base / polish / theme），不按主题拆 | 原文件里 818 行之后是一层覆盖规则，重新声明了前面同一批选择器。按主题拆会打散这层覆盖、改变层叠顺序，静默产生视觉回归 |
| 样式用多个 `<link>` 输出，不用 `@import` | `@import` 里的 URL 无法带版本参数（CSS 是静态文件），而 nginx 给 .css 发了 7 天缓存 —— 改了样式用户最长 7 天看不到，且无任何提示 |
